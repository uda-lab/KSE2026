...
| 8 | codex プラグインの broker/app-server チェーン残留（14 系統，~1.5GB）により Lean ビルドが OOM 連発（cgroup ~3.4GiB）．sweep・headroom 確認・`.lake` ハードリンク共有・`flock` 直列化の恒常規約を導入 | 2026-07-18 | RESOURCE / RECOVERY | **primary**（EV-0247 = session `de129390`，EV-0005．7/18 vps 3 本の符号化済: `evidence/session-index/de129390…md` ほか） | INC-005 |
