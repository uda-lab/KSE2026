...
| 8 | codex プラグインの broker/app-server チェーン残留（14 系統，~1.5GB）により Lean ビルドが反復死（exit 137，cgroup ~3.4GiB．当時は OOM 連発と診断されたが，kill 機構の OOM 帰属は 2026-07-31 の #100 再調査で否定・未特定に訂正）．sweep・headroom 確認・`.lake` ハードリンク共有・`flock` 直列化の恒常規約を導入 | 2026-07-18 | RESOURCE / RECOVERY | **primary**（EV-0247 = session `de129390`，EV-0005．7/18 vps 3 本の符号化済: `evidence/session-index/de129390…md` ほか．#100 の host journald 照合を追補） | INC-005 |
| 9 | **ログローテーション（`cleanupPeriodDays` 30 日既定）による研究記録の不可逆喪失**．創成期 6/10〜6/14 を全 host で喪失（local-secondary 上での削除が有力）．VPS 7/10 スナップショット復元試行は「VPS には当該期間の記録が元々無い」ことの確定という負の結果．`cleanupPeriodDays=9999`（2026-07-21）で以後停止．メタ・インシデント | 2026-06〜07 | LOSS / RECOVERY | primary（`analysis/vps-snapshot-20260710-verification.md`，escrow 全域） | INC-003 |
