...
3. 2026-06-20T04:18–04:43Z — `uda-lab-agent`（null チャネル）がこれをプログラム構造へ
   ...
   `leray-hopf#2`–`leray-hopf#6` を派生させる（`leray-hopf#7` は同系列だが
   06:17:43Z であり，この窓の外）．*primary／高．*
...
### T3 — 投稿主体・経路・指示元を揃えて自己開示した唯一の記録と，それが記録する再演（**drift + 是正**）
...
4. 10:24:08Z — `leray-hopf#177` 上に，**投稿主体（ChatGPT）と経路（認証済み connector）と
   指示元（owner の明示的指示）を同時に自己開示した唯一の記録**が投稿される．
   独立レビュー欠落という事実自体は 72 秒前の `leray-hopf#178` の本文（10:22:56Z）が
   既に述べており，投稿環境の制約という意味でのチャネル言及も同日 09:42:26Z の
   `leray-hopf#157` コメント（T9）が先行している．本コメントに固有なのは
   **3 者を揃えて明示している点**である．同コメントの要旨は，当該投稿が owner の
...
初稿は「要求された英文の文言も含めて」と書いていたが，そのままでは誤りである．指示は
「…程度へ弱めてください」という**強度の上限**の指定であり，本文の逐語指定ではない．
本文（`paper/sections/04-incidents.tex`）に採用されたのは強度である．ただし
`claims/paper-claims.md:82` の CLM-005 Notes には指示が例示した英文が
`a contrast consistent with the value of independent statement review` として
**逐語で**記録されている．「本文への逐語採用はないが，claim 台帳の Notes には
逐語で残っている」が正確な記述である．
...
`leray-hopf#64` の作成のみである．**投稿主体・経路・指示元を揃えて自己開示した唯一の記録（T3）と
