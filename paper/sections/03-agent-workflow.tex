...
reports a selected subset of event families used in this paper's control
analysis, not the complete coded vocabulary; the full indices remain in the
evidence repository. The selected families connect the evidence to the design:
statement and mathematical events call for semantic gates, whereas ownership,
handoff, and resource events call for operational and review-completion
controls.
% CLAIM: CLM-004

\begin{table}[t]
\centering
\caption{Selected event counts in the six purposively selected sessions.}\label{tab:event-codes}
