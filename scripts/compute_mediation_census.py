...
AUTHORSHIP_SIGNAL_CAVEAT = (
    ...
    "complete census, AND the word 'direct' is ambiguous between two "
    "readings that this data answers differently — state which one is "
    "meant before quoting a bound:\n"
    "(1) 'direct' = posted without a GitHub App intermediary. Under this "
    "reading, performed_via_github_app: null is an EXACT count of that "
    "property, and — since any PAT-holding process (gh CLI, an "
    "orchestrator, a harness) also yields null — it is an UPPER BOUND on "
    "human-typed comments (some or all of the null count may be "
    "process-posted).\n"
    "(2) 'direct' = the human composed the text. Under this reading, null "
    "bounds nothing in either direction: a connector-attributed comment can "
    "carry text the human wrote verbatim, and a null comment can carry text "
    "an agent generated. Verifiable primary evidence for the latter: "
    ...
    "performed_via_github_app null and matching a repeated, formulaic "
    "workflow-notice pattern (consistent with automated posting, though "
    "this data alone cannot prove who or what posted them).\n"
    "Conversely, a non-null performed_via_github_app slug IS a LOWER BOUND "
    "on mediated (connector-transmitted) authorship under reading (1), but "
    "proves only the transport channel under reading (2) — not who "
    "composed the prose, nor whether the human read it before it was "
    "posted. No client, user-agent, IP, session id, model version, or "
    "prompt is recoverable from this data. Comment edit history is not "
    "retroactively available, and deleted/minimized comments are absent "
    "from the export. Do not infer productivity, effectiveness, or "
    "causation from these counts."
)

CAVEAT_REF = (
    "count_exact is an exact tally, not itself a bound; 'direct' is "
    "ambiguous between two readings this data answers differently "
    "(no-App-intermediary vs. human-composed-the-text) — "
    "see authorship_signal_caveat in mediation-census.json / "
    "analysis/mediation-census-methodology.md before quoting a bound"
)
