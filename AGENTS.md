...
- `make selftest` — gate 機構自体の回帰テスト（`scripts/test_ci_gates.sh`，84 ケース）．
  ...
  到達できない assertion は `ok` ではなく `skip` と報告する（chktex 不在時は 76 pass + skip 6 ブロック）．
  snapshot exporter の name-denylist scrub（denylist 不在時に不活性・存在時に literal マスク・
  identifier フィールド非マスク）も検査対象．
