...
| 2026-07-25 | scientific owner latest-head review | PR #59 head `a4d3350` | scientific owner | Vlasov の線形・非線形分類，novelty claim の過大な範囲，Section III の時制，推定コストと session time の用語，局所的な数学・Lean・incident 表現，leray-hopf PR 参照，PR description の head を指摘．別監査事項は issue #60 へ分離 | Vlasov を非線形と訂正し，novelty を 3 次元 Navier–Stokes の Leray–Hopf 弱解存在へ限定．end-state synthesis を一度明示し，費用・時間・局所用語・repository citation を校正．issue #60 は本修正の対象外 |
| 2026-07-25 | additional agent review and owner triage | PR #59 head `fe6a94c` | Claude Code (Opus 5) review + scientific owner | listing 内の `%` による prose scanner fail-open，per-PR guard と release attestation の混同，PR #177 再発の脱落を should-fix とし，claim location，abstract map，citation 改行，未使用 macro，copy/reference を同期候補として指摘 | owner が8項目を本 PR で採用．scanner 回帰テスト，attestation 区別，PR #177 一文，claim/citation/copyedit 同期を実装．4件は owner 判断で approval blocker から除外 |
