...
| 2026-07-23 | Claude Fable 5 (Claude Code) | issue #42 項目 2/3/4 の実装: extract_usage.py に campaign end 07-20 導入（除外 46 レコード，$6,972.83/199.1h に再集計），CLM-003 の FX 下界条件付き化（raw export ラベル内訳不存在の確認は owner 立会いの計画セッションで実施），CLM-004/005 の主張強度校正，「ゾンビ」用語の残留チェーン系への統一，FALSE-SUCCESS 定義注記 (issue #43) | scripts/extract_usage.py，usage-metrics.json 再生成，claims/paper-claims.md，analysis 4 文書，INC-005・session-index 用語修正 | codex 内部レビュー + codex 独立レビュー + merge gate 検査．校正方針は owner 指定（issue #42・チャット判断 2026-07-23） |
