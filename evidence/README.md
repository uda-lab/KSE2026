...
- `manifest.csv` — 収集した証拠アイテムの台帳．`scripts/inventory_sessions.py` が
  ...
  `mtime_utc`，`stat_ctime_utc`（Unix では inode 変更時刻であり**作成時刻ではない**），
  `session_id`（推定），`tool_or_model`，
...
- `redacted-excerpts/` — redaction 済み抜粋のみ．**手で選別した Markdown に限る**
  （`.jsonl` はリポジトリ全域でコミット禁止．CI が強制．正規化 JSONL は
  `private/derived/` に置く）．置く前に
